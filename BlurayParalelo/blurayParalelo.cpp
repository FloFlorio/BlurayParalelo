	...
	for (int i = 0; i < n; i++) {
        for (int c1 = 0; c1 <= capacity[0]; c1++) {
            for (int c2 = 0; c2 <= capacity[1]; c2++) {
                    for (int c3 = 0; c3 <= capacity[2]; c3++) {
                        if (file_size[i] <= c1) {
                            memo[c1 - file_size[i]][c2][c3] = max(memo[c1 - file_size[i]][c2][c3], memo[c1][c2][c3] + file_size[i]);
                	}
                	    if (file_size[i] <= c2 && nCapacities >= 2) {
                        	memo[c1][c2 - file_size[i]][c3] = max(memo[c1][c2 - file_size[i]][c3], memo[c1][c2][c3] + file_size[i]);
                	}
                	    if (file_size[i] <= c3 && nCapacities == 3) {
                        	memo[c1][c2][c3 - file_size[i]] = max(memo[c1][c2][c3 - file_size[i]], memo[c1][c2][c3] + file_size[i]);
                	}
        	}
                	}
        	}
